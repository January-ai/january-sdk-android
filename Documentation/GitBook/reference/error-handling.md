...
or `date_range_too_large` (a start date more than five years back); both are
`ErrorCategory.VALIDATION`. So is `conflict` (HTTP 409), a request that clashes
with what the API holds: it is permanent, and sending it again unchanged gets the
same answer.
