...
internal fun DemoSetupScreen() {
    Column(Modifier.fillMaxSize().safeDrawingPadding().verticalScroll(rememberScrollState()).padding(horizontal = DemoScreenPadding).padding(vertical = 40.dp), verticalArrangement = Arrangement.spacedBy(24.dp)) {
        ...
        Text("To use this demo app, initialize the SDK in code with your token provider or a local development API key.", color = JanuaryColors.Body)
        DemoCard {
            SetupOption("1", "Use your token provider", "Recommended", "Set january.partnerTokenUrl and january.partnerSessionToken in local.properties.")
            HorizontalDivider(Modifier.padding(vertical = 18.dp), color = JanuaryColors.Divider)
            SetupOption("2", "Use a development API key", "Debug only", "Set january.apiKey in local.properties. Never commit or ship it.")
        }
        DemoCard {
            SectionLabel("Where to configure")
            Text("local.properties", style = MaterialTheme.typography.titleMedium)
            Text("Edit the local configuration file, then build again.", fontSize = 15.sp, color = JanuaryColors.Body)
        }
    }
}
