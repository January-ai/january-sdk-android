...
class RestaurantMenuResourceTest {
    ...
    @Test fun menuByIdUsesPaginationAndPreservesNutrition(): Unit = runBlocking {
        ...
        try {
            server.enqueue(MockResponse().setHeader("Content-Type", "application/json").setBody("""{"items":[{"id":"101","name":"Bowl","nutrients":{"calories":{"value":220,"unit":"kcal"}},"glycemic_index":null,"glycemic_load":null,"servings":[{"id":"11","quantity":1,"unit":"bowl","scaling_factor":1,"weight_grams":null,"is_primary":true}]}]}"""))
    ...
    @Test fun menuDetailsPreserveNestedNutritionImagesAndEveryServing(): Unit = runBlocking {
        ...
        try {
            server.enqueue(MockResponse().setHeader("Content-Type", "application/json").setBody("""
                {"items":[{"type":"menu_item","id":"menu-item-42","name":"Oatmeal","restaurant_name":"Cafe","is_chain":null,
                "nutrients":{"calories":{"value":200,"unit":"kcal"},"protein":{"value":8,"unit":"g"},"carbohydrates":{"value":35,"unit":"g"},"total_fat":{"value":4,"unit":"g"}},
                "glycemic_index":45,"glycemic_load":12,"image_url":"https://example.com/oatmeal.png","distance_meters":12,
            ...
            assertEquals(listOf("11", "12"), item.servings.map { it.id?.value })
            assertEquals(0.14175, item.servings.last().scalingFactor, 0.0)
            assertEquals(28.35, item.servings.last().weightGrams!!, 0.0)
            assertTrue(item.servings.first().isPrimary == true)
            assertFalse(item.servings.last().isPrimary == true)
        } finally { server.shutdown() }
    }
}
